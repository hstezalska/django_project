...
#from folder_projektu.folder_aplikacji import serializers
...
def osoba_search(request, substring): 
    osoby = Osoba.objects.filter(imie_icontains = substring) | Osoba.objects.filter(nazwisko_icontains = substring)
    serializer = OsobaSerializer(osoby, many = True)
    return Response(serializer.data)
